...
app.whenReady().then(async () => {
  ...
  if (TAG === 'real') {
    ...
    // ---- S5: 回选 fx-alpha → 只读面消失(常驻审批接管) ----
    const back = await js(win, `(() => { const r = document.querySelector('.sidebar-row'); if (!r) return false; r.click(); return true })()`)
    ...
    mark('s5', back && readonlyGone && approvalBack, 'S5 回选父会话 → 只读面消失(审批接管)')
