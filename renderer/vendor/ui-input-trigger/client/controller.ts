...
/** zion patch: bail 事件面(官方 ClientContext.bail 的类型面)。 */
export interface TriggerActxShim {
...
export interface InputTriggerControllerDeps {
   ...
   * zion patch: 官方是 cordis ClientContext(bail 事件面来自官方输入机 contract,
   * 不在 zion 编译面);本地化为 execute() 消费的 bail 面。 */
  actx: TriggerActxShim
