...
function navigableMembers(
  ...
  // zion patch: zion 的 useSessions 绑 manager 快照,无 ids/byId 面;容错为空
  // (成员行退化为非导航态,面板结构/状态不受影响)。
  const list = sessions as unknown as { ids?: readonly string[]; byId?: Record<string, { origin?: string; parentId?: string; running?: boolean }> }
  const ordinary = new Set(list.ids ?? [])
  const result: SessionId[] = []
  for (const phase of phases) {
    for (const member of phase.members) {
      const summary = list.byId?.[member.childId]
...
export function WorkflowRunPanel({ node, sessionId, useSessions, openSession, t }: WorkflowRunPanelProps) {
  // zion patch: ChatConversationViewNode.data 是 unknown,按定义窄化。
  const data = node.data as WorkflowRunChatData
  const totalMembers = data.phases.reduce((count, phase) => count + phase.members.length, 0)
  const requiresExpansion = data.status !== 'completed'
    || data.phases.some(phaseRequiresExpansion)
  const navigable = useSessions(
    sessions => navigableMembers(sessions, data.phases, sessionId),
    shallowEqual,
  )
  return (
    <section className={css.root} data-workflow-run data-run-status={data.status}>
      <RunHeader
        count={totalMembers}
        name={data.name}
        requiresExpansion={requiresExpansion}
