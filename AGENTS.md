...
按 [`docs/zion-ui-visual-inventory.md`](./docs/zion-ui-visual-inventory.md) 逐块把 pi-martix-ui-dev 的 UI 迁移进本仓时:

1. **风格默认 = 保持原样**:除非用户明确要求按 `MatrixDesign-minimal.md` 或 `DESIGN.md` 重新设计,迁移块**尽量保持 pi-martix-ui-dev 中原有的风格**逐样搬入,不做极简/电影化改写(此时上方的「未指明默认极简」不适用——迁移任务的默认是原样迁移)。
2. **先对位,再迁移**:迁移每个块前,先在 [`docs/ui-component-inventory.md`](./docs/ui-component-inventory.md) 中找到它所属的目标 UI 组件;**先生成一个补齐缺失功能入口的 UI demo**(迁移视觉 + 目标组件应有的全部功能入口),确认后再落进真组件。
3. **多余入口先问后删**:如果 pi-martix-ui-dev 的块里有、而 `ui-component-inventory.md` 中不存在的功能入口,**必须先询问用户是否删除这些入口,得到确认后再迁移**。删除范围仅限从 pi-martix-ui-dev 迁移过来的这些组件上的入口;`ui-component-inventory.md` 中列出的任何功能入口**一律不得删除**。
