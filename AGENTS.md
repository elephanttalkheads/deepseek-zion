...
> ⚠️ **当前阶段暂停适用。** 本工程目前仍处于「**复刻官方 web UI**」阶段——目标是先把官方 dsh web 的展示内容与交互入口**1:1 复刻齐全**,尚不是「在完整 UI 上做风格化改造」。因此本条铁律(不删复刻的展示内容)暂时**不适用**;在做复刻补齐/对齐官方 UI 时,不要求「先记 `ui-change-log` 再删改」。
>
> **恢复条件**:当复刻已覆盖官方 dsh web 的核心展示内容与交互入口(对照官方 UI 验收达标)之后,进入「风格化改造/换肤/动效/布局调整」阶段时,本条铁律恢复生效,再按下方规则执行。
...
- **冻结/暂停说明**:本条规则在当前「复刻官方 web UI」阶段被显式暂停(见文首 ⚠️),暂停期内的删改无需走 `ui-change-log`;**恢复条件满足时自动解冻**。
